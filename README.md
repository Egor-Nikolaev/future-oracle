...
Крупные движения кластеризуются. Обучил **логистическую регрессию** (несколько окон
реализованной волатильности + давность последнего всплеска) на train, проверил на test:
**lift ~1.6x, recall ~53%** (ловит половину крупных движений). Простой порог vol7 даёт выше
lift (2.2x), но recall всего ~24% — компромисс precision/recall честно показан в отчёте
`npm run backtest`. Модель выдаёт **вероятность крупного движения** на каждой карточке.

**Кросс-валидация (устойчивость edge).** Единичный сплит легко обманывает, поэтому прогнал
**walk-forward CV на 4 временных окнах**: lift **1.36x ± 0.17** (диапазон 1.12–1.57), recall ~45%.
Edge **положителен во всех окнах** (даже худшее 1.12x > 1), но скромнее оптимистичного single-split
(1.6x). В UI показываю именно CV-число — честный, а не приукрашенный.

**Ablation funding.** Проверил гипотезу «funding-экстремумы предсказывают волатильность»:
добавил funding-историю Binance как фичу. **Не помогает** (lift 1.50x против 1.59x без неё) —
честно отброшено, а не притянуто за уши. Funding остаётся в карточке как контекст
позиционирования, но НЕ как валидированный предиктор.

**Ablation интрадей.** Проверил гипотезу «внутридневная волатильность помогает»: построил из
часовых данных (90д, самосогласованно) внутридневную реализованную волатильность как фичу.
**Помогает** — на 90-дневном окне lift 1.37x → **1.46x**, recall +3пп. Гипотеза подтвердилась
(в отличие от funding), поэтому интрадей-модель **используется для live-риска**, когда часовые
данные доступны. Ограничение честное: бесплатный CoinGecko даёт часовые только за 90 дней.

Продукт честно показывает **риск-режим** (низкий/средний/высокий) и вероятность крупного
движения, а не обещает угадать up/down.
...
13. **Измерил вместо того, чтобы гадать.** Вместо накрутки точности построил честный harness на
    ...
    волатильность. Это и есть главная правка: не тюнить шум под направление, а вынести то,
    что работает. Отрицательный результат оставлен как есть.
14. **Логрег-модель волатильности + честный ablation funding.** Углубил рабочий сигнал:
    обучил логистическую регрессию на нескольких фичах (окна волатильности, давность всплеска) —
    даёт вероятность крупного движения и вдвое выше recall, чем одиночный порог. Гипотезу
    «funding-экстремумы помогают» проверил тем же харнессом: **не подтвердилась** (lift не вырос),
    честно отброшена. Так и надо: не каждая фича проходит проверку.
15. **Шрифты самохостом ради детерминированного билда.** `next/font/google` иногда не мог скачать
    Orbitron с Google Fonts на билде Vercel и валил деплой. Перевёл оба шрифта на `next/font/local`
    (файлы в репо) — сборка больше не зависит от сети, редеплой не падает.
16. **Регион Франкфурт (fra1) из-за геоблока Binance.** funding/OI не тянулись на проде: Binance
    Futures отдаёт 451 из US-дата-центров Vercel. Локально работало, на проде — нет. Перенёс
    функции в fra1 — оттуда Binance доступен. Классический баг «работает у меня».
17. **Кросс-валидация сдула оптимизм.** Единичный train/test сплит хвастался lift 1.6x. Прогнал
    walk-forward CV на нескольких временных окнах — **1.36x ± 0.17** (edge положителен во всех окнах,
    но скромнее). В UI поставил CV-число, а не приукрашенное. Один сплит может повезти — CV честнее.
18. **Интрадей-волатильность: гипотеза подтвердилась, в отличие от funding.** Проверил тем же
    ablation'ом: построил внутридневную волатильность из часовых данных (90д, самосогласованно).
    **Помогает** (lift 1.37→1.46, recall +3пп) — поэтому интрадей-модель используется для live-риска,
    когда часовые данные есть. Итог всех проверок: 2 гипотезы подтвердились (логрег-волатильность,
    интрадей), 2 отброшены (направление, funding) — и всё показано цифрами, а не на веру.

19. **Сверка с реальностью и её слепое пятно.** Скоррелировал прогнозы с фактическим движением
    за последние дни: направление ~50% (r≈0, монетка, ожидаемо), а модель волатильности проспала
    недавние прорывы ИЗ ТИШИНЫ (recall 0/8) — она ловит продолжение волатильного периода, но не
    взрыв из спокойствия. По факту: добавил фичу сжатия волатильности (vol7/vol30 + серия тихих
    дней, эффект сквиза), она прошла проверку (CV lift 1.36→1.45, recall на прорывах 0→9%) и
    оставлена. Плюс вынес самопроверку в интерфейс: приложение честно сверяет себя с реальностью
    за последние 10 дней (направление угадано %, крупных движений поймано N/M). Взрыв из тишины
    по одной цене предсказать почти нельзя, и это показано, а не спрятано.
