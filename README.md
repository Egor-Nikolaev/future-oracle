...
Два механизма:
1. **Live-сверка:** каждый прогноз хранит `base_price` и направление; на следующем обновлении
   сравнивается с фактической ценой (`actual_dir`, `hit`).
2. **Walk-forward бэктест** (`lib/backtest.js`, наверху страницы) — главная честная метрика:
   прогноз в день T строится из данных ДО T и сверяется с движением T→T+1, по 40 дням реальной
   истории CoinGecko, **со сравнением с наивным бейзлайном** («всегда самый частый класс»).

**Честный результат, который я не прячу:** ценовой сигнал на бэктесте **не бьёт бейзлайн**
(порядка 42% против 62%). Краткосрочное направление крипты близко к случайному, и это видно
в цифрах, а не заметается под ковёр. Ценность приложения — прозрачность и проверяемость метода,
а не «высокая точность»; поэтому доходность не обещается (и ТЗ этого требует). Бэктест на
коротком окне (40 дней, ~130 прогнозов, один рыночный режим) — не приговор модели, но честно
показывает: edge над бейзлайном тут нет.
...
12. **Устойчивость к лимитам API.** CoinGecko free строгий: markets + история для бэктеста легко
    ловят 429. Добавил ретрай с бэкоффом, бэктест на холодном старте не гоняю (беру из seed,
    кнопка пересчитывает), предохранитель против записи пустого seed.
