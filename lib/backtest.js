...
// Часовые цены (CoinGecko days<=90 отдаёт hourly). Возвращает [[ts_ms, price], ...].
async function hourlyPrices(id, days = 90) {
  const data = await cgGet(`${BASE}/coins/${id}/market_chart?vs_currency=usd&days=${days}`);
  return data.prices || [];
}

// Из часовых строим ПО ДНЯМ: дневное закрытие + внутридневную реализованную волатильность
// (std часовых доходностей внутри суток). Всё из одного источника — без кросс-выравнивания.
function dailyFromHourly(hourly) {
  const byDay = new Map();
  for (const [ts, price] of hourly) {
    const d = new Date(ts).toISOString().slice(0, 10);
    if (!byDay.has(d)) byDay.set(d, []);
    byDay.get(d).push(price);
  }
  const days = [...byDay.keys()].sort();
  const closes = [], intradayVol = [];
  for (const d of days) {
    const p = byDay.get(d);
    closes.push(p[p.length - 1]); // закрытие дня
    const rets = [];
    for (let i = 1; i < p.length; i++) rets.push(((p[i] - p[i - 1]) / p[i - 1]) * 100);
    intradayVol.push(std(rets)); // внутридневная волатильность
  }
  return { closes, intradayVol };
}
...
function featurize(prices, funding = []) {
  ...
  for (let t = 30; t < prices.length - 1; t++) { // t от 30: нужна 30-дневная база для сжатия
    const chg24h = rets[t];
    ...
    const volAccel = vol3 / (vol14 + 0.1); // ускорение волатильности (недавняя / долгая)
    let ewma = 0, wsum = 0;                 // EWMA-волатильность (свежее весит больше)
    for (let k = 0; k < 14 && t - k >= 1; k++) { const wk = Math.pow(0.9, k); ewma += wk * rets[t - k] ** 2; wsum += wk; }
    const ewmaVol = Math.sqrt(ewma / (wsum || 1));
    // СЖАТИЕ волатильности: долгая тишина предшествует прорыву (сквиз/эффект Боллинджера).
    const vol30 = volOf(rets, t, 30);
    const compression = vol30 > 0 ? vol7 / vol30 : 1; // <1 = тихо относительно долгой нормы
    let calm = 0; for (let k = 0; k < 20 && t - k >= 1; k++) { if (Math.abs(rets[t - k]) < 1) calm++; else break; }
    const f = [vol3, vol7, vol14, Math.abs(rets[t]), since, volAccel, ewmaVol, compression, calm];
...
// Walk-forward кросс-валидация волатильностной модели: несколько временных окон
// (expanding train → следующий кусок test). Возвращает mean±std lift/recall — проверка,
// что edge устойчив, а не удача одного сплита. cuts — доли обучающей части.
function crossValidateVol(perAssetRows, key = "f", cuts = [0.4, 0.5, 0.6, 0.7]) {
  const lifts = [], recalls = [];
  for (const cut of cuts) {
    const train = [], test = [];
    for (const id of Object.keys(perAssetRows)) {
      const rows = perAssetRows[id] || [];
      const a = Math.floor(rows.length * cut);
      const b = Math.floor(rows.length * (cut + 0.2)); // окно теста ~20%
      train.push(...rows.slice(0, a));
      test.push(...rows.slice(a, b));
    }
    if (train.length < 30 || test.length < 20) continue;
    const v = evalVolatilityModel(train, test, key);
    if (v.lift != null) lifts.push(v.lift);
    if (v.recall != null) recalls.push(v.recall);
  }
  return {
    folds: lifts.length,
    lift_mean: lifts.length ? Number(mean(lifts).toFixed(2)) : null,
    lift_std: lifts.length ? Number(std(lifts).toFixed(2)) : null,
    recall_mean: recalls.length ? Number(mean(recalls).toFixed(2)) : null,
    lift_min: lifts.length ? Number(Math.min(...lifts).toFixed(2)) : null,
    lift_max: lifts.length ? Number(Math.max(...lifts).toFixed(2)) : null,
  };
}

// Интрадей-ablation: помогает ли внутридневная волатильность предсказывать крупные
// движения сверх дневных фич? Всё строим из часовых данных (90д, самосогласованно).
async function intradayAblation(ids, cache) {
  const rows = [];
  const lastByCoin = {}; // последний интрадей-вектор фич по коину (для live-риска)
  const lastVolByCoin = {};
  for (let i = 0; i < ids.length; i += 2) {
    ...
    await Promise.all(batch.map(async (id) => {
      ...
      try {
        let hourly;
        if (cache && cache[key] && Date.now() - cache[key].at < CACHE_TTL_H * 3.6e6) {
          hourly = cache[key].hourly;
        } else {
          hourly = await hourlyPrices(id, 90);
          if (cache && hourly.length) cache[key] = { at: Date.now(), hourly };
        }
        const { closes, intradayVol } = dailyFromHourly(hourly);
        const base = featurize(closes);
        base.forEach((r, idx) => {
          const t = 14 + idx;
          const iv = intradayVol[t] ?? 0, ivPrev = intradayVol[t - 1] ?? 0;
          const withIntra = { ...r, fIntra: [...r.f, iv, ivPrev] };
          rows.push(withIntra);
          lastByCoin[id] = withIntra.fIntra;     // перезапишется последним днём
          lastVolByCoin[id] = Number((intradayVol[t] ?? 0).toFixed(2));
        });
      } catch { /* пропускаем коин */ }
    }));
  }
  if (rows.length < 60) return { available: false };
  const cut = Math.floor(rows.length * 0.6);
  const train = rows.slice(0, cut), test = rows.slice(cut);
  const noIntra = evalVolatilityModel(train, test, "f");
  const withIntra = evalVolatilityModel(train, test, "fIntra");
  const helps = (withIntra.lift || 0) > (noIntra.lift || 0) + 0.05;
  return {
    available: true,
    window_days: 90,
    n: test.length,
    without_intraday: { lift: noIntra.lift, recall: noIntra.recall },
    with_intraday: { lift: withIntra.lift, recall: withIntra.recall },
    helps,
    // для live: модель с интрадей + последние фичи и внутридневная волатильность по коинам
    model: helps ? { coef: withIntra.coef, thr_prob: withIntra.thr_prob, lastByCoin, lastVolByCoin } : null,
  };
}

// Самопроверка за последние N дней: как модель показала себя против факта.
// Направление, доля попаданий (ожидаемо ≈ монетка, честно). Волатильность, сколько
// крупных движений (>3%) модель пометила высоким риском заранее (recall на свежих днях).
// Использует ДНЕВНУЮ модель волатильности (coef на фичах `f`) — консистентно с perAssetRows.
function recentPerformance(perAssetRows, volCoef, thrProb, days = 10) {
  let dirTot = 0, dirHit = 0, big = 0, bigFlagged = 0, flags = 0;
  for (const id of Object.keys(perAssetRows)) {
    const rows = perAssetRows[id] || [];
    for (const r of rows.slice(-days)) {
      const dir = directionOf(combineScore({ momentum: r.momentum }));
      dirTot++;
      if (dir === r.actualDir) dirHit++;
      const prob = volCoef ? applyLogistic(volCoef, r.f) : null;
      const flagged = prob != null && prob >= thrProb;
      if (flagged) flags++;
      if (r.bigMove) { big++; if (flagged) bigFlagged++; }
    }
  }
  return {
    days,
    n: dirTot,
    direction_hit_rate: dirTot ? Number((dirHit / dirTot).toFixed(3)) : null,
    big_moves: big,
    big_moves_flagged: bigFlagged,
    volatility_recall: big ? Number((bigFlagged / big).toFixed(3)) : null,
...
export async function runBacktest(days = 365) {
  ...
  const cv = crossValidateVol(perAssetRows, featKey); // устойчивость edge по нескольким окнам
  const recent = recentPerformance(perAssetRows, volPrice.coef, volPrice.thr_prob, 10); // сверка с последними днями
  let intraday = { available: false };
  try { intraday = await intradayAblation(ids, cache); } catch { /* интрадей best-effort */ }
  saveCache(cache);

  // интрадей-модель (если подтвердилась ablation'ом) даёт лучший риск — используем её
  const im = intraday.model;
  const per_asset = {};
  for (const id of ids) {
    const rows = perAssetRows[id] || [];
    const last = rows.length ? rows[rows.length - 1] : null;

    if (im && im.lastByCoin[id]) { // приоритет: интрадей-модель
      const prob = applyLogistic(im.coef, im.lastByCoin[id]);
      const high = prob >= im.thr_prob;
      per_asset[id] = {
        realized_vol: im.lastVolByCoin[id] ?? (last ? Number(last.realizedVol.toFixed(2)) : null),
        prob_big_move: Number(prob.toFixed(3)),
        risk: high ? "high" : prob >= im.thr_prob * 0.6 ? "medium" : "low",
    ...
    let prob = null, high;
    if (useModel) { prob = applyLogistic(coef, last[featKey]); high = prob >= chosen.thr_prob; }
    else high = last.realizedVol > volSimple.threshold;
    per_asset[id] = {
  ...
  return {
    ...
    volatility_cv: cv,                            // кросс-валидация: устойчив ли lift
    recent,                                       // самопроверка за последние ~10 дней
    intraday_ablation: intraday,                  // помогает ли внутридневная волатильность
