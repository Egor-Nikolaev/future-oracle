...
function migrate(db) {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS price_snapshots (
      ...
      chg_24h    REAL,               -- % за 24ч
      chg_7d     REAL,               -- % за 7д
      funding    REAL,               -- funding rate (Binance Futures), позиционирование плечей
      open_interest REAL,            -- открытый интерес (размер плеча в рынке)
    ...
    -- Разное «ключ-значение»: результат walk-forward бэктеста и т.п.
...
export function insertSnapshot(s) {
  const info = getDb()
    .prepare(`INSERT INTO price_snapshots (asset_id, price, volume, chg_24h, chg_7d, funding, open_interest, fetched_at)
              VALUES (@asset_id, @price, @volume, @chg_24h, @chg_7d, @funding, @open_interest, @fetched_at)
              ON CONFLICT(asset_id, fetched_at) DO NOTHING`)
    .run({ funding: null, open_interest: null, ...s });
  return info.lastInsertRowid;
}

// --- meta (key-value) ---
export function setMeta(key, valueObj) {
  getDb()
