...
export function confidenceOf({ score, momentum, sentiment, newsCount, chg24h }) {
...
export function risksOf({ momentum, sentiment, newsCount, chg24h, score }) {
  const risks = [];
  ...
  if (newsCount === 0)
    risks.push("Нет свежих новостей по активу — прогноз опирается только на цену.");
  else if (newsCount < 2)
...
export function driversOf({ chg24h, chg7d, sentiment, newsCount, momentum }) {
  const d = [];
...
// Собрать полный прогноз из снимка и списка сентиментов новостей по активу.
// sentiments — массив чисел [-1..1]; пустой = нет новостного сигнала.
export function buildPrediction({ snapshot, sentiments = [] }) {
  ...
  const confidence = confidenceOf({ score, momentum, sentiment, newsCount, chg24h });
  const risks = risksOf({ momentum, sentiment, newsCount, chg24h, score });
  const drivers = driversOf({ chg24h, chg7d, sentiment, newsCount, momentum });
  return { momentum, sentiment, newsCount, score, direction, confidence, risks, drivers };
}
